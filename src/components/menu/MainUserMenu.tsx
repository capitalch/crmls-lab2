							...
							{userMenuRoutes.length > 0 && (
								<div className="py-1">
									{userMenuRoutes.map(({ id, title, path, target }, i) => {
										return (
											<NavLink key={i} exact className="text-secondary hover:text-primary hover:bg-secondary group flex items-center px-4 py-2 text-sm font-medium rounded-sm" to={{ pathname: path }} activeClassName="bg-secondary text-primary" target={target ?? ""} rel="noopener noreferrer">
												{title}
											</NavLink>
										);
									})}
