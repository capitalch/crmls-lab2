	...
	return (
		<>
			<div className="flex flex-col flex-shrink-0" data-tip={`<p class="text-md font-semibold">${application.name}</p>${(application.hoverText) ? '<p class="text-xs">' + application.hoverText + '</p>' : ''}`}>
				{application.iconUrl ? (
					<>
						<div className={`p-0 sm:px-3 sm:py-2 cursor-pointer rounded-xl shadow-md border border-divider flex items-center justify-center sm:justify-start w-16 sm:w-64 h-16 ${application.mlsSystem ? "bg-crmls-blue bg-opacity-20 border border-2 border-header" : "bg-white bg-opacity-10"} overflow-hidden relative app-card-${application.applicationCategory?.toLowerCase() ?? 'none'}`} onClick={() => handleAppClick(application)}>
